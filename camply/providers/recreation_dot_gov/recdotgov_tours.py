...
class RecreationDotGovTours(RecreationDotGovBase):
        ...
        for matching_date, date_related_data in campsite_data.facility_availability_summary_view_by_local_date.items():
            for (
                ...
                if availability_status.reservable > 0:
                    booking_url = cls.booking_url.format(**vars(availability_status))
                    booking_date = datetime.combine(matching_date, time.min)
                    try:
    ...
    def get_campground_ids_by_campsites(
        ...
        if unknown_ids:
            if len(campground_ids_unique) != 1:
                raise ProviderSearchError("With unknown tours, all known ones must be from the same faciity.")
            for unknown_id in unknown_ids:
                campsite = TourResponse(
                    TourID=unknown_id,
                    FacilityID=campground_ids_unique[0],
                    TourName=f"Tour #{unknown_id}",
